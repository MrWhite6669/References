...
{
    class ModifyBullets : GlobalProjectile
    {

        public bool IsBullet(Projectile projectile)
        {
            if (projectile.arrow || projectile.melee || projectile.minion || projectile.sentry || projectile.thrown || projectile.magic || projectile.aiStyle > 0) return false;
            return true;
        }

        public bool IsMagic(Projectile projectile)
        {
            if (projectile.magic) return true;
            return false;
        }

        public override void AI(Projectile projectile)
        {
            #region Homing Bullets
            Player player = Main.player[projectile.owner];
            if (player.GetModPlayer<BattleUtilitiesModPlayer>().bulletHomingEffect && IsBullet(projectile) && player.GetModPlayer<BattleUtilitiesModPlayer>().btdCharged)
            {
            ...
            if (player.GetModPlayer<BattleUtilitiesModPlayer>().magicHomingEffect && IsMagic(projectile) && player.GetModPlayer<BattleUtilitiesModPlayer>().atdCharged)
            {
                float num132 = (float)Math.Sqrt((double)(projectile.velocity.X * projectile.velocity.X + projectile.velocity.Y * projectile.velocity.Y));
                float num133 = projectile.localAI[0];
                if (num133 == 0f)
